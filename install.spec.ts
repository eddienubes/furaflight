...
describe("install.cjs platform detection", () => {
  ...
  it("should always point the destination at bin/furaflight.exe", () => {
    expect(install.DEST_BINARY_NAME).toBe("furaflight.exe");
    expect(install.getDestPath().endsWith(`bin${require("node:path").sep}furaflight.exe`)).toBe(
      true,
    );
  });
